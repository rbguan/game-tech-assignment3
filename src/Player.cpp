...
void Player::_register_methods()
{
    ...
    register_method("_update_health_bar", &Player::_update_health_bar, GODOT_METHOD_RPC_MODE_DISABLED);
...
void Player::_rifle_right()
{
    Rifle* rifle = static_cast<Rifle*>(get_node("Rifle"));
    rifle->set_position(Vector2(abs(rifle->get_position().x), rifle->get_position().y));
    rifle->set_flip_h(false);
}

void Player::_rifle_left()
{
    Rifle* rifle = static_cast<Rifle*>(get_node("Rifle"));
    rifle->set_position(Vector2(-abs(rifle->get_position().x), rifle->get_position().y));
    rifle->set_flip_h(true);
}

void Player::_update_health_bar()
{
    static_cast<TextureProgress*>(get_node("GUI/HealthBar"))->set_value(healthPoints);
}
...
void Player::_die()
{
    ...
    static_cast<Rifle*>(get_node("Rifle"))->set_process(false);
...
void Player::_on_RespawnTimer_timeout()
{
    ...
    static_cast<Rifle*>(get_node("Rifle"))->set_process(true);
